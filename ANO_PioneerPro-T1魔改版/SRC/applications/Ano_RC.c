...
void unlock(u8 dT_ms) {
  if (flag.power_state <= 2 &&
      para_sta.save_trig == 0)  //只有电池电压非最低并且没有操作flash时，才允许进行解锁
  {
  ...
  if (CH_N[CH_THR] < -UN_THR_VALUE) {
    //判断用户是否想要上锁、解锁
    if (ABS(CH_N[CH_YAW]) > 0.1f * UN_YAW_VALUE &&
        CH_N[CH_PIT] < -0.1f * UN_PIT_VALUE) {
      if (flag.locking == 0) {
        flag.locking = 1;
      }
    } else {
    ...
    //飞控上锁、解锁检测
    if (CH_N[CH_PIT] < -UN_PIT_VALUE && CH_N[CH_ROL] > UN_ROL_VALUE &&
        CH_N[CH_YAW] < -UN_YAW_VALUE) {
      stick_fun_0 = 1;
      flag.locking = 2;
    } else if (CH_N[CH_PIT] < -UN_PIT_VALUE && CH_N[CH_ROL] < -UN_ROL_VALUE &&
               CH_N[CH_YAW] > UN_YAW_VALUE) {
      stick_fun_0 = 1;
      flag.locking = 2;
    } else {
    ...
    stick_function_check_longpress(dT_ms, &unlock_f, unlock_time, stick_fun_0,
                                   f, &flag.unlock_cmd);
  } else {
    flag.locking = 0;  //油门高
  ...
  if (flag.start_ok) {
    ...
    if (RC_IN_MODE == PWM) {
      for (u8 i = 0; i < CH_NUM; i++) {
        if (chn_en_bit &
            (1
             << i))  //(Rc_Pwm_In[i]!=0)//该通道有值，==0说明该通道未插线（PWM）
        {
          CH_N[i] = 1.25f * ((s16)Rc_Pwm_In[i] -
      ...
      for (u8 i = 0; i < CH_NUM; i++) {
        ...
        {
          CH_N[i] = 1.25f * ((s16)Rc_Ppm_In[i] -
                             1100);  // 700 -- 1500us,处理成大约+-500摇杆量
        } else {
          CH_N[i] = 0;
        }
        CH_N[i] = LIMIT(CH_N[i], -500, 500);  //限制到+—500
    ...
    } else  // sbus
    {
      for (u8 i = 0; i < CH_NUM; i++) {
        ...
        {
          CH_N[i] = 0.65f * ((s16)Rc_Sbus_In[i] -
                             1024);  // 248 --1024 --1800,处理成大约+-500摇杆量
        } else {
          CH_N[i] = 0;
        }
        CH_N[i] = LIMIT(CH_N[i], -500, 500);  //限制到+—500
...
void fail_safe_check(u8 dT_ms)  // dT秒调用一次
{
  ...
  {
    cnt = 0;
    if ((chn_en_bit & 0x0F) != 0x0F ||
        flag.chn_failsafe)  //前4通道有任意一通道无信号或者受到接收机失控保护信号
    {
      cnt2++;
    } else {
      cnt2--;
