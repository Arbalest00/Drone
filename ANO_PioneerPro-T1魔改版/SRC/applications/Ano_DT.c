...
u8 data_to_send_2[50];
u8 checkdata_to_send, checksum_to_send;
u8 send_user_data_flag = 0;
u8 sendchar_test_1[8] = {0xaa,0xaa,0xaa,0xaa,0xaa,0xaa,0xaa,0xaa};
/////////////////////////////////////////////////////////////////////////////////////
...
void ANO_DT_Data_Exchange(void) {
  ...
  if ((cnt % senserSta_cnt) == (senserSta_cnt - 1)) f.send_senserSta = 1;
	...
	if ((cnt % test) == (test - 1)) f.send_test = 1;

  if ((cnt % user_cnt) == (user_cnt - 1)) f.send_user = 1;

  if ((cnt % status_cnt) == (status_cnt - 1)) f.send_status = 1;

  if ((cnt % rcdata_cnt) == (rcdata_cnt - 1)) f.send_rcdata = 1;

  if ((cnt % motopwm_cnt) == (motopwm_cnt - 2)) f.send_motopwm = 1;

  if ((cnt % power_cnt) == (power_cnt - 2)) f.send_power = 1;
...
//    ANO_User_Recall_Data();
  ...
  /////////////////////////////////////////////////////////////////////////////////////
    ...
    for (u8 i = 0; i < CH_NUM; i++) {
      if ((chn_en_bit &
           (1 << i)))  //(Rc_Pwm_In[i]!=0 || Rc_Ppm_In[i] !=0  )//该通道有值
      {
    ...
    ANO_DT_Send_RCData(CH_GCS[2], CH_GCS[3], CH_GCS[0], CH_GCS[1], CH_GCS[4],
                       CH_GCS[5], CH_GCS[6], CH_GCS[7], 0, 0);
  }
  /////////////////////////////////////////////////////////////////////////////////////
  else if (f.send_motopwm) {
    f.send_motopwm = 0;
#if MOTORSNUM == 8
    ANO_DT_Send_MotoPWM(motor[0], motor[1], motor[2], motor[3], motor[4],
  ...
  /////////////////////////////////////////////////////////////////////////////////////
    ...
    f.send_location = 0;
    ANO_DT_Send_Location(switchs.gps_on, Gps_information.satellite_num,
                         (s32)Gps_information.longitude,
                         (s32)Gps_information.latitude, 123, 456);
    ...
    if (*(data_buf + 3) == 0XE0)  //命令E0
    {
      switch (*(data_buf + 5))  // FUN
      {
        ...
        case 0x02:
          ...
          if (*(data_buf + 6) == 0x00 &&
              *(data_buf + 7) == 0xAB)  //恢复默认参数
          {
            Parame_Reset();
            data_save();
          }
          if (*(data_buf + 6) == 0x00 &&
              *(data_buf + 7) == 0xAF)  //恢复所有参数
          {
            PID_Rest();
            All_PID_Init();
            Parame_Reset();
        ...
        case 0xE1:
          f.paraToSend =
              (u16)(*(data_buf + 6) << 8) | *(data_buf + 7);  //读取参数
          break;
        case 0x10:
      ...
      ANO_DT_SendCmd(SWJADDR, *(data_buf + 5),
                     (u16)(*(data_buf + 6) << 8) | *(data_buf + 7),
                     (u16)(*(data_buf + 8) << 8) | *(data_buf + 9),
                     (u16)(*(data_buf + 10) << 8) | *(data_buf + 11),
                     (u16)(*(data_buf + 12) << 8) | *(data_buf + 13),
                     (u16)(*(data_buf + 14) << 8) | *(data_buf + 15));
    } else if (*(data_buf + 3) == 0XE1)  //设置参数
    {
      u16 _paraNum = (u16)(*(data_buf + 5) << 8) | *(data_buf + 6);
      s32 _paraVal =
          (s32)(((*(data_buf + 7)) << 24) + ((*(data_buf + 8)) << 16) +
                ((*(data_buf + 9)) << 8) + (*(data_buf + 10)));
...
//发送到上位机的PID参数全部是*1000后的
void ANO_DT_ParListToParUsed(void) {
  Ano_Parame.set.pid_att_1level[ROL][KP] =
      (float)ParValList[PAR_PID_1_P] / 1000;
  Ano_Parame.set.pid_att_1level[ROL][KI] =
      (float)ParValList[PAR_PID_1_I] / 1000;
  Ano_Parame.set.pid_att_1level[ROL][KD] =
      (float)ParValList[PAR_PID_1_D] / 1000;
  Ano_Parame.set.pid_att_1level[PIT][KP] =
      (float)ParValList[PAR_PID_2_P] / 1000;
  Ano_Parame.set.pid_att_1level[PIT][KI] =
      (float)ParValList[PAR_PID_2_I] / 1000;
  Ano_Parame.set.pid_att_1level[PIT][KD] =
      (float)ParValList[PAR_PID_2_D] / 1000;
  Ano_Parame.set.pid_att_1level[YAW][KP] =
      (float)ParValList[PAR_PID_3_P] / 1000;
  Ano_Parame.set.pid_att_1level[YAW][KI] =
      (float)ParValList[PAR_PID_3_I] / 1000;
  Ano_Parame.set.pid_att_1level[YAW][KD] =
      (float)ParValList[PAR_PID_3_D] / 1000;

  Ano_Parame.set.pid_att_2level[ROL][KP] =
      (float)ParValList[PAR_PID_4_P] / 1000;
  Ano_Parame.set.pid_att_2level[ROL][KI] =
      (float)ParValList[PAR_PID_4_I] / 1000;
  Ano_Parame.set.pid_att_2level[ROL][KD] =
      (float)ParValList[PAR_PID_4_D] / 1000;
  Ano_Parame.set.pid_att_2level[PIT][KP] =
      (float)ParValList[PAR_PID_5_P] / 1000;
  Ano_Parame.set.pid_att_2level[PIT][KI] =
      (float)ParValList[PAR_PID_5_I] / 1000;
  Ano_Parame.set.pid_att_2level[PIT][KD] =
      (float)ParValList[PAR_PID_5_D] / 1000;
  Ano_Parame.set.pid_att_2level[YAW][KP] =
      (float)ParValList[PAR_PID_6_P] / 1000;
  Ano_Parame.set.pid_att_2level[YAW][KI] =
      (float)ParValList[PAR_PID_6_I] / 1000;
  Ano_Parame.set.pid_att_2level[YAW][KD] =
      (float)ParValList[PAR_PID_6_D] / 1000;
  ...
  Ano_Parame.set.pid_gps_loc_1level[KP] =
      (float)ParValList[PAR_PID_11_P] / 1000;
  Ano_Parame.set.pid_gps_loc_1level[KI] =
      (float)ParValList[PAR_PID_11_I] / 1000;
  Ano_Parame.set.pid_gps_loc_1level[KD] =
      (float)ParValList[PAR_PID_11_D] / 1000;
  Ano_Parame.set.pid_gps_loc_2level[KP] =
      (float)ParValList[PAR_PID_12_P] / 1000;
  Ano_Parame.set.pid_gps_loc_2level[KI] =
      (float)ParValList[PAR_PID_12_I] / 1000;
  Ano_Parame.set.pid_gps_loc_2level[KD] =
      (float)ParValList[PAR_PID_12_D] / 1000;
...
void ANO_DT_Send_VER(void) {
  ...
  temp[12] = BL_VER % 256;
  u8 check_sum = 0;
  for (u8 i = 0; i < 13; i++) check_sum += temp[i];
  temp[13] = check_sum;

  ANO_DT_Send_Data(temp, 14);
}

void ANO_DT_Send_Version(u8 hardware_type, u16 hardware_ver, u16 software_ver,
                         u16 protocol_ver, u16 bootloader_ver) {
  u8 _cnt = 0;
  data_to_send[_cnt++] = 0xAA;
  data_to_send[_cnt++] = MYHWADDR;
  data_to_send[_cnt++] = SWJADDR;
...
void ANO_DT_Send_Location(u8 state, u8 sat_num, s32 lon, s32 lat,
                          float back_home_angle, float back_home_dist) {
  u8 _cnt = 0;
  vs16 _temp;
  vs32 _temp2;
  u16 _temp3;
...
void ANO_User_Recall_Data(void) {
  ...
  /*-------------------------------------
	...
	[0]0XAA 
	[1]0x55
	[2]flag.unlock_sta         解锁状态 0为上锁状态 1为解锁状态
	[3]flag.auto_take_off_land 起飞状态 0为AUTO_TAKE_OFF_NULL 起飞前  1为AUTO_TAKE_OFF 起飞中   2为AUTO_TAKE_OFF_FINISH 起飞完成   3为AUTO_LAND 降落
	[4]flag.ct_alt_hold        高度稳定 当高度与设定高度相差5cm时为1，代表高度稳定，否则为0（有问题）
	[5][6][7][8]wcz_hei_fus.out高度*1000
	[9][10]imu_data.yaw        航向 本身应该是+-180范围 在传输时将角度加了180°，并且在AttCtrl中也有点问题，yaw轴的pid运算反馈值在原本的程序中写成了0，我已经改成了反馈值，不知道改得对不对
	[11][12]loc_ctrl_1.fb[X]   X速度 应该是PIT轴速度
	[13][14]loc_ctrl_1.fb[Y]   Y速度 应该是ROL轴速度
	[15][16]loc_ctrl_1.fb[Z]   Z速度 应该是YAW轴速度
	[17]                       求和校验位
	--------------------------------------*/
  data_to_send_2[_cnt++] = 0xAA;  // head
  data_to_send_2[_cnt++] = 0x55;  // head

  data_to_send_2[_cnt++] = flag.unlock_sta;
  data_to_send_2[_cnt++] = flag.auto_take_off_land;
  data_to_send_2[_cnt++] = flag.ct_alt_hold;

  _temp2 = (int)(wcz_hei_fus.out * 1000);
  data_to_send_2[_cnt++] = BYTE3(_temp);
  data_to_send_2[_cnt++] = BYTE2(_temp);
	data_to_send_2[_cnt++] = BYTE1(_temp);
  data_to_send_2[_cnt++] = BYTE0(_temp);
  _temp = (int)((imu_data.yaw + 180) * 100);
  data_to_send_2[_cnt++] = BYTE1(_temp);
  data_to_send_2[_cnt++] = BYTE0(_temp);
  ...
  data_to_send_2[2] = _cnt - 3;  // calc length

  u8 sum = 0;
  for (u8 i = 0; i < _cnt; i++) sum += data_to_send_2[i];

  data_to_send_2[_cnt++] = sum;  // sumcheck

//  ANO_User_Send_Data(data_to_send_2, _cnt);
//	Uart5_Send(data_to_send_2, _cnt);
}
