...
void Auto_Take_Off_Land_Task(u8 dT_ms)  //
{
  ...
  if (flag.auto_take_off_land == AUTO_TAKE_OFF) {
    ...
    auto_taking_off_speed =
        AUTO_TAKE_OFF_KP *
        (Ano_Parame.set.auto_take_off_height - wcz_hei_fus.out);
    //计算起飞速度
    auto_taking_off_speed = LIMIT(auto_taking_off_speed, 0, max_take_off_vel);

    //退出起飞流程条件1，满足高度或者流程时间大于5000毫秒。
    if (take_off_ok_cnt >= 5000 ||
        (Ano_Parame.set.auto_take_off_height - loc_ctrl_2.exp[Z] <
         2))  //(auto_ref_height>AUTO_TAKE_OFF_HEIGHT)
    {
      flag.auto_take_off_land = AUTO_TAKE_OFF_FINISH;
    }
    //退出起飞流程条件2，2000毫秒后判断用户正在控制油门。
    if (take_off_ok_cnt > 2000 &&
        ABS(fs.speed_set_h_norm[Z]) >
            0.1f)  // 一定已经taking_off,如果还在推杆，退出起飞流程
    {
      flag.auto_take_off_land = AUTO_TAKE_OFF_FINISH;
    }
  ...
  if (flag.auto_take_off_land == AUTO_LAND) {
    //设置自动下降速度
    auto_taking_off_speed =
        -(s16)LIMIT(Ano_Parame.set.auto_landing_speed, 20, 200);
  }
}
...
void Alt_2level_Ctrl(float dT_s) {
  ...
  loc_ctrl_2.exp[Z] =
      LIMIT(loc_ctrl_2.exp[Z], loc_ctrl_2.fb[Z] - 200, loc_ctrl_2.fb[Z] + 200);
  //
  if (pc_user.engage_height_set) {
    loc_ctrl_2.exp[Z] = pc_user.height_set;
    pc_user.engage_height_set = 0;
  }
  //
  ...
  if (fs.alt_ctrl_speed_set != 0) {
    ...
    if (ABS(loc_ctrl_1.exp[Z] - loc_ctrl_1.fb[Z]) < 5) {
      flag.ct_alt_hold = 1;
    }
  }
...
void Alt_1level_Ctrl(float dT_s) {
  ...
  loc_ctrl_1.exp[Z] =
      0.6f * fs.alt_ctrl_speed_set + alt_val_2.out;  //速度前馈0.6f，直接给速度

  w_acc_z_lpf += 0.1f * (imu_data.w_acc[Z] - w_acc_z_lpf);  //低通滤波

  loc_ctrl_1.fb[Z] =
      wcz_spe_fus.out + Ano_Parame.set.pid_alt_1level[KD] *
                            w_acc_z_lpf;  //微分先行，下边PID函数微分系数为0
  ...
  if (flag.taking_off == 1) {
    LPF_1_(1.0f, dT_s, THR_START * 10,
           err_i_comp);  // err_i_comp = THR_START *10;
  } else {
    err_i_comp = 0;
  }
